...
def IBMSlistMaker(promo):
    ...
    startDate = premiereDate.date() - timedelta(
        14
    )  # fecha inicio para avances y puntuales -solo dia
    avEndDate = premiereDate.date() - timedelta(
        1
    )  # que el avance deje de salir un día antes del estreno
    startGenDate = promo["genStartDate"].date()  # fecha de inicio de la gen -solo dia
    startGenDateTime = promo[
        "genStartDate"
    ]  # fecha de inicio de la gen -dia y hora, para la peli del mes de EE
    endGenDate = promo["endDate"].date()  # fecha fin de las promos gen
    bumpEndDate = date(2022, 12, 31)  # fecha de fin para los bumps

    approvDeadline = returnWeekday(startDate - timedelta(9))  # para plani Seguimiento
    txtDeadline = returnWeekday(startDate - timedelta(15))  # para plani Seguimiento
    ...
    casosRedux = (
        ...
        "NT",
        "CAPS ESTRENO",
    )  # casos en lo que aplica hacer un redux a cont
    ...
    feedLang = "SPA"
    strAvance = unidecode(
        properDay(premiereDate, feedLang)
        + " "
    ...
    if showFeed == "EGSUR" or showFeed == "EGNOR":
        strAvance = unidecode(
            ...
            + str(premiereDate.day)
            + " de "
            + str(properMonth(premiereDate, feedLang))
            + " "
            + str(properTime(premiereDate, showFeed, dstMexFlag, dstChiFlag))
        )
        bumpDuration = 5
        ...
        avSuffix = (
            " "
            + unidecode(
                properDay(premiereDate, feedLang) + " " + str(premiereDate.day)
            ).upper()
        )
        todayStr = "HOY"
        strHoy = (
        ...
        showFeed == "AMCSUR"
        or showFeed == "AMCNORCOL"
        or showFeed == "AMCLATAM"
    ...
    if promoPckg == "ESTRENO" or promoPckg == "NT" or promoPckg == "CAPS ESTRENO":
        # el Avance
        avanceIBMS = [
            promoCode.upper() + " " + str(promoPckg) + " " + avSuffix,
            secondsToTC(0),
            secondsToTC(duration),
            secondsToTC(duration),
            strAvance.upper(),
            feedStr,
            "",
        ...
        genIBMS = [
            promoCode.upper() + " " + str(promoPckg) + " " + genSuffix,
            secondsToTC(0),
            secondsToTC(duration),
            secondsToTC(duration),
            strGen.upper(),
        ...
        if (
            ...
            hoyGenIBMS = [
                promoCode.upper() + " " + todayStr + " " + genSuffix,
                secondsToTC(0),
                secondsToTC(duration),
                secondsToTC(duration),
                strHoy.upper(),
            ...
            if (
                ...
                avanceIBMS = [
                    ...
                    + str(promoPckg)
                    + " "
                    + str(properDay(premiereDate, feedLang).upper())
                    + " "
                    + str(premiereDate.day),
                    secondsToTC(0),
                    secondsToTC(duration),
                ...
                manIBMS = [
                    promoCode.upper() + " " + unidecode(tomorrowStr),
                    secondsToTC(0),
                    secondsToTC(duration),
                    secondsToTC(duration),
                    strMan.upper(),
                    ...
                    manStartDate,
                    manStartDate,
                    returnWeekday(manStartDate - timedelta(7)),
                ]
                ...
                manGenIBMS = [
                    promoCode.upper() + " " + genSuffix + " " + unidecode(tomorrowStr),
                    secondsToTC(0),
                    secondsToTC(duration),
                    secondsToTC(duration),
                    strManGen.upper(),
        ...
        if (
            ...
            outputIBMS.extend(
                [avanceIBMS, manIBMS, hoyIBMS, genIBMS, manGenIBMS, hoyGenIBMS]
            )

    elif promoPckg == "PUNTUAL":
        # Calcula la fecha de venta, segun si es o no en el mismo mes que se emite
        if (
            premiereDate.month != startDate.month
        ):  # Si la puntual sale al aire el mes anterior, decimos mes.
            strPuntual = unidecode(
                properDay(premiereDate, feedLang)
                + " "
                + str(premiereDate.day)
                + " de "
        ...
        strGen = unidecode("PROMO PREMIO " + str(properMonth(premiereDate, feedLang)))
        genClub = [
            promoCode.upper(),
            secondsToTC(0),
            secondsToTC(duration),
        ...
        strHoy = unidecode(
            todayStr + str(properTime(premiereDate, showFeed, dstMexFlag, dstChiFlag))
        )
        hoyIBMS = [
            promoCode.upper() + " " + todayStr,
            secondsToTC(0),
        ...
        strPuntual2 = unidecode(
            properDay(startGenDateTime, feedLang)
            + " "
            + str(startGenDateTime.day)
            + " "
            + str(properTime(startGenDateTime, showFeed, dstMexFlag, dstChiFlag))
        )
        puntualIBMS2 = [
            promoCode.upper()
            + " "
            + str(properDay(startGenDateTime, feedLang).upper())
            + " "
            + str(startGenDateTime.day),
            secondsToTC(0),
            secondsToTC(duration),
        ...
        if showFeed == "EGSUR" or showFeed == "MCLATAM":
            ...
            bumpAContIBMS = [
                "A CONT-" + (str(showName)).upper(),
                secondsToTC(0),
                secondsToTC(bumpDuration),
                secondsToTC(bumpDuration),
                "A CONTINUACION",
                bumpFeedStr,
                " ",
            ...
            # Bumps a cont
            ahoraComienzaIBMS = [
                unidecode(ahoraComienzaPreffix + (str(showName)).upper()),
                secondsToTC(0),
                secondsToTC(bumpDuration),
                secondsToTC(bumpDuration),
                "NEXT",
            ...
            ensegVolvemosIBMS = [
                unidecode(ensegVolvemosPreffix + (str(showName)).upper()),
                secondsToTC(0),
                secondsToTC(bumpDuration),
                secondsToTC(bumpDuration),
                "STAY TUNED",
            ...
            yaVolvimosIBMS = [
                unidecode(yaVolvimosPreffix + (str(showName)).upper()),
                secondsToTC(0),
                secondsToTC(bumpDuration),
                secondsToTC(bumpDuration),
                "NOW",
            ...
            outputIBMS.extend([ahoraComienzaIBMS, ensegVolvemosIBMS, yaVolvimosIBMS])
        else:
            errorMsg = [
                "Disculpe, el tipo de promo "
                + str(promoPckg)
                + " solicitado para la promo "
                + promoCode.upper()
                + " no esta disponible para la señal "
                + str(showFeed)
            ]
    ...
    if (
        showFeed == "EGSUR" and promoPckg in casosRedux
    ):  # Redux a cont - caso especial para EGSUR y ESTRENO
        reduxAContIBMS = [
            promoCode.upper() + " REDUX A CONT",
            secondsToTC(0),
            secondsToTC(15),
            secondsToTC(15),
            "A CONTINUACION",
